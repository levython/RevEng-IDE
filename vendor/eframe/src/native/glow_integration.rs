...
impl GlutinWindowContext {
    ...
    unsafe fn new(
        ...
        */
        ...
        // If ES also fails (e.g. old drivers without WGL_EXT_create_context_es2_profile),
        // try a compatibility profile as a last resort — works on any OpenGL 2.1+ system.
        ...
        let compat_context_attributes = glutin::context::ContextAttributesBuilder::new()
        ...
        let gl_context = match gl_context_result {
            ...
            Err(err) => {
                ...
                match gles_result {
                    Ok(it) => it,
                    Err(err) => {
                        log::warn!("Failed to create GLES context: {err}. Retrying with compatibility profile.");
                        unsafe {
                            gl_config
                                .display()
                                .create_context(&gl_config, &compat_context_attributes)?
                        }
                    }
                }
            }
